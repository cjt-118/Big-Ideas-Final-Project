# **Big Ideas – Final Project**

This repository contains the final project for **Group 7** in the University of Pittsburgh’s *Big Ideas* course.  
Our goal was to determine which Pittsburgh neighborhood can be considered the **"best"** based on a combination of quantitative metrics from publicly available data.

# Group Name - Heroes of Pittsburgh

---

## ** Team Members & Metrics**

### **Roger Torres**  
- **Email:** ROT91@pitt.edu  
- **Metric / Notebook:** *Economic Advantage Score*

### **Caleb J. Teeters**  
- **Email:** CJT118@pitt.edu  
- **Metric / Notebook:** *Non-Traffic Citations*

### **Pavan Ulaganathan**  
- **Email:** PAU12@pitt.edu  
- **Metric / Notebook:** *Fire Incidents*

### **Nick Foster**  
- **Email:** NLF52@pitt.edu  
- **Metric / Notebook:** *Crimes*

---

## ** Project Description**

To determine the **"best" neighborhood in Pittsburgh**, our group analyzed four different metrics:

- **Economic Advantage Score** – Measures aggregate household income from the last 12 months (ACS 2014 data).  
- **Non-Traffic Citations** – Counts non-traffic citations issued per neighborhood.  
- **Fire Incidents** – Reports fire-related incidents across various blocks in the city.  
- **Crime Data** – Tracks violent crimes per 1,000 residents in each neighborhood.

By comparing these metrics across neighborhoods, we aimed to identify which areas demonstrate the strongest combination of safety, stability, and economic opportunity.

---

## ** Datasets Used**

- **Economic Advantage (Aggregate Household Income)**  
  [ACS 2014 Household Income Dataset](https://data.wprdc.org/dataset/pittsburgh-american-community-survey-2014-miscellaneous-data/resource/41d215a7-6baf-4de8-8593-d25c858d3546)

- **Fire Incidents**  
  [Pittsburgh Fire Incidents Dataset](https://data.wprdc.org/dataset/fire-incidents-in-city-of-pittsburgh/resource/8d76ac6b-5ae8-4428-82a4-043130d17b02)

- **Non-Traffic Citations**  
  [Non-Traffic Citations in Pittsburgh](https://data.wprdc.org/dataset/non-traffic-citations)

- **Crimes**  
  *(Do not have link)*
